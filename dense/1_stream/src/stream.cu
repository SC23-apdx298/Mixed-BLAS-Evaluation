...
template <typename T>
void print_row_elem(T t, const int &width)
{
...
int main(int argc, char **argv)
{
  ...
  string_buffer >> h_num_elems;

  // Device specific memory operations
  real *d_one, *d_two, *d_three;
  size_t d_num_bytes = sizeof(real) * h_num_elems;
  ...
  dim3 thread_blocks(h_num_elems / threads_per_block.x);

  if (h_num_elems % threads_per_block.x != 0)
  {
    thread_blocks.x += 1;
  }

  // Initialize data on device
  initialize_device_buffer(thread_blocks, threads_per_block, h_num_elems, d_one, 2.0);
  initialize_device_buffer(thread_blocks, threads_per_block, h_num_elems, d_two, 0.5);
  initialize_device_buffer(thread_blocks, threads_per_block, h_num_elems, d_three, 0.5);

  std::unordered_map<std::string, std::function<void(dim3, dim3, size_t, real *, real *, real *)>> kernel_func_map{
      {"copy", {stream_copy}},
      {"scale", {stream_scale}},
      {"add", {stream_add}},
      {"triad", {stream_triad}}};
  ...
  for (const auto &kernel : kernel_func_map)
  {
    for (int i = 0; i < num_iters + 1; i++)
    {
      ...
      kernel.second(thread_blocks, threads_per_block, h_num_elems, d_one, d_two, d_three);
  ...
  char outname[80];
  sprintf(outname, "../data/stream_4090_%lu.csv", sizeof(real) * 8);
  outfile.open(outname, std::ios::app);
  // outfile.open("doube_stream.csv", std::ios::app);
  // outfile.open("float_stream.csv", std::ios::app);
  // outfile.open("half_stream.csv", std::ios::app);
  outfile << h_num_elems << ",";
  outfile << block_x << ",";
  ...
  for (const auto &kernel_time : timer_map)
  {
    ...
    double avg_time = std::accumulate(kernel_time.second.begin() + 1, kernel_time.second.end(), 0.0) / num_iters;
    double bw = 1e-9 * size_map[kernel_time.first] / (*min_max.first);
    std::cout << "| ";
