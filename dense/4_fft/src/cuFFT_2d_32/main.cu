...
int main(int argc, char **argv) {
    ...
    sig_idx[0] = 1 << m;
    sig_idx[1] = 1 << n;
    long long sig_size = sig_idx[0] * sig_idx[1];
    ...
    size_t ws = 0;
    r = cufftXtMakePlanMany(plan, 2, sig_idx, NULL, 1, 1, CUDA_C_32F, NULL, 1,
                            1, CUDA_C_32F, 1, &ws, CUDA_C_32F);
    assert(r == CUFFT_SUCCESS);
    WARMUP
    ...
    printf("2d,32,%lld,%f,%lf,%lld,%lld\n", sig_size, et, gflops, sig_idx[0], sig_idx[1]);
