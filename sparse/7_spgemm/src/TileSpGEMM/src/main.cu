...
int main(int argc, char ** argv)
{
	...
	if (argc < 6)
    {
        INFO_LOG("Run the code by './test -d 0 -aat 0 matrix.mtx'.\n");
        return 0;
    }
    ...
    read_Dmatrix(&matrixA->m, &matrixA->n, &matrixA->nnz, &matrixA->rowpointer, &matrixA->columnindex, &csrValA_tmp, &matrixA->isSymmetric, filename);
    ...
    matrixA->value = (MAT_VAL_TYPE *)malloc(sizeof(MAT_VAL_TYPE) * matrixA->nnz);
    for (size_t i = 0; i < matrixA->nnz; i++)
    {
        matrixA->value[i] = (MAT_VAL_TYPE)csrValA_tmp[i];
    }
    free(csrValA_tmp);

    if (!aat &&  matrixA->m != matrixA->n)
    {
        INFO_LOG("matrix squaring must have rowA == colA. Exit.\n");
        return 0;
    }

    INFO_LOG("the tilesize = %d\n",BLOCK_SIZE);
    ...
    if (aat)
    {
        ...
        {
           INFO_LOG("matrix AAT does not do symmetric matrix. Exit.\n");
           return 0;
        }
    ...
    else
    {
        ...
        INFO_LOG("CSR to Tile conversion uses %.2f ms\n", time_conversion);
#endif
...
INFO_LOG("tile space overhead = %.2f MB\n", mem);
    ...
    // if (fout == NULL)
    //     INFO_LOG("Writing results fails.\n");
...
INFO_LOG("-------------------------------check----------------------------------------\n");
tile2csr(matrixC);
        INFO_LOG("tile to CSR conversion complete!\n");
