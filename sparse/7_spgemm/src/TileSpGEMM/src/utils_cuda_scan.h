                              ...
                              const  int  lane_id)
{
    ...
    __syncthreads();
    //if (print_tag && !warp_id) INFO_LOG("before s_warpsync[%i] = %i\n", lane_id, s_warpsync[lane_id]);
    //__syncthreads();
    ...
    __syncthreads();
    //if (print_tag && !warp_id) INFO_LOG("after s_warpsync[%i] = %i\n", lane_id, s_warpsync[lane_id]);
    //__syncthreads();
    ...
    //if (print_tag) INFO_LOG("step 2 key[%i] = %i\n", warp_id*WARP_SIZE+lane_id, key[warp_id*WARP_SIZE+lane_id]);
    //__syncthreads();
                                ...
                                const int  length)
{
    // struct timeval tv;
/*
    INFO_LOG("exclusive_scan_device_cuda, size = %i, start\n", length);
...
/*
    ...
    INFO_LOG("exclusive_scan_device_cuda, size = %i, RUNTIME = %4.2f ms\n", length, time);
*/
}
