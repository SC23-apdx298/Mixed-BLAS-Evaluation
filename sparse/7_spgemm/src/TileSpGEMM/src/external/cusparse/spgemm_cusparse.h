...

{
    ...
    //  - cuda SpGEMM start!
    INFO_LOG(" - cuda SpGEMM start! Benchmark runs %i times.\n", BENCH_REPEAT);

    if (check_result && BENCH_REPEAT > 1)
    {
        INFO_LOG("If check_result, Set BENCH_REPEAT to 1.\n");
        return -1;
    }
    ...
    INFO_LOG(" - cuda SpGEMM completed!\n\n");
    ...
    *gflops_segmerge = 2 * (double)nnzCub / (1e6 * time_cuda_spgemm);
    INFO_LOG("nnzC = %i, nnzCub = %lld, Compression rate = %4.2f\n",
           *nnzC, nnzCub, *compression_rate);
    INFO_LOG("CUDA  cuSPARSE SpGEMM runtime is %4.4f ms, GFlops = %4.4f\n",
    ...
    if (check_result)
    {
        ...
        else
        {
            ...
            if (*nnzC != nnzC_golden)
            {
                ...
                INFO_LOG("[NOT PASSED] nnzC = %i, nnzC_golden = %i\n", *nnzC, nnzC_golden);
            }
            else
            {
                INFO_LOG("[PASSED] nnzC = %i\n", *nnzC);
            }
            ...
            if (errcounter != 0)
            {
                INFO_LOG("[NOT PASSED] row_pointer, #err = %i\n", errcounter);
            }
            else
            {
                INFO_LOG("[PASSED] row_pointer\n");
            }
            ...
            /*for (int i = 0; i < mC; i++)
        {
            ...
            for (int j = 0; j < *nnzC; j++)
            {
                ...
                {
                    //    INFO_LOG("h_csrColIdxC[j] = %i,  h_csrColIdxC_golden[j] = %i\n",h_csrColIdxC[j] ,h_csrColIdxC_golden[j]);
                    errcounter++;
                }
            }
            ...
            if (errcounter != 0)
            {
                INFO_LOG("[NOT PASSED] column_index & value, #err = %i (%4.2f%% #nnz)\n",
                       errcounter, 100.0 * (double)errcounter / (double)(*nnzC));
            }
            else
            {
                INFO_LOG("[PASSED] column_index & value\n");
            }
