                ...
                double *time_step1, double *time_step2, double *time_step3, double *time_malloc)
{
    ...
    gettimeofday(&t1, NULL);
        ...
        *time_step1 += (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
        if (ri == 0)
        {
            INFO_LOG("step1 ----Calculate the number and tile-column index of tiles of matrixC---\n");
            INFO_LOG("step1 ---------------------- Runtime is  %.2f ms-------------------------\n", *time_step1);
        }
        ...
        {
            INFO_LOG("d_blkcsr_Ptr_C failed\n");
        }
        ...
        *time_step2 = (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
        if (ri == 0)
        {
            ...
            INFO_LOG("step2 ---------------------- Runtime is  %.2f ms-------------------------\n", *time_step2);
        }
#endif
        ...
        *time_step3 = (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
        if (ri == 0)
        {
            ...
            INFO_LOG("step3 ---------------------- Runtime is  %.2f ms------------------------\n", *time_step3);
            INFO_LOG("\n-----------------------Malloc uses %.2f ms-------------------------------\n", *time_malloc);
        }
    ...
    *gflops_tile = 2.0 * (double)nnzCub / (tile_spgemm_time * 1e6);
    ...
    INFO_LOG("Non-empty tiles of C = %i\n", numblkC);
    INFO_LOG("nnzC = %i\n", nnzC);
    INFO_LOG("CUDA  TileSpGEMM runtime is %4.2f ms, gflops = %4.2f\n", tile_spgemm_time, *gflops_tile);
    printf("%lu,%lu,%lu,%d,%d,%lu,%lf,%lf,%lf\n", sizeof(MAT_VAL_TYPE), sizeof(MAT_VAL_TYPE), sizeof(MAT_VAL_TYPE), matrixA->m, matrixA->n, nnzCub, tile_spgemm_time, *gflops_tile, *compression_rate);
