...
template <typename T>
CSR<T> loadCSRbiio(const char *file)
{
	int row, col, nnz;
	int *row_ptr;
	int *col_idx;
	...
	CSR<T> res;
	res.alloc(row, col, nnz);

	for (size_t i = 0; i < nnz; i++)
	{
		res.col_ids[i] = col_idx[i];
		res.data[i] = val[i];
	}
	for (size_t i = 0; i < row + 1; i++)
	{
		res.row_offsets[i] = row_ptr[i];
	}
	free(row_ptr);
	free(col_idx);
	free(val);
...
template CSR<float> loadCSRbiio(const char *file);
template CSR<double> loadCSRbiio(const char *file);
