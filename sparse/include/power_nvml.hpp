...
std::vector<unsigned int> power_his;
...
void *powerPollingFunc(void *ptr)
{
	...
	while (pollThreadStatus)
	{
		...
		if (pmmode == NVML_FEATURE_ENABLED)
		{
			...
			power_his.push_back(powerLevel);
		}
...
void nvmlAPIRun()
{
	...
	nvmlResult = nvmlDeviceGetCount_v2(&deviceCount);
	if (NVML_SUCCESS != nvmlResult)
	{
	...
	{
		// Get the device ID.
		nvmlResult = nvmlDeviceGetHandleByIndex_v2(i, &nvmlDeviceID);
		if (NVML_SUCCESS != nvmlResult)
		{
		...
		nvmlResult = nvmlDeviceGetPciInfo_v3(nvmlDeviceID, &nvmPCIInfo);
		if (NVML_SUCCESS != nvmlResult)
		{
	...
	nvmlResult = nvmlDeviceGetHandleByIndex_v2(0, &nvmlDeviceID);
...
double get_avg_power_usage()
{
	double avg = 0;
	for (size_t i = 0; i < power_his.size(); i++)
	{
		avg += power_his[i];
	}
	avg = (power_his.size() == 0) ? 0 : avg / power_his.size();
	return avg;
}
